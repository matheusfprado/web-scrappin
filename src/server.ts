...
function getMercadoLivreSelectors(): ScrapeSelectors {
  return {
...
function buildMercadoLivreUrl(niche?: string): string {
  if (!niche) return "https://lista.mercadolivre.com.br/";
  const slug = niche
    .toLowerCase()
    .trim()
    .replace(/\s+/g, "-")
...
function calculateFinalPrice(price: number, discountText: string): number {
...
function matchesNiche(title: string, niche?: string): boolean {
  if (!niche) return true;
  return title.toLowerCase().includes(niche.toLowerCase());
}

async function scrapeProducts(payload: ScrapeRequest): Promise<ProductResult[]> {
  const selectors =
    payload.selectors ??
  ...
  if (!waitAttempts.some(Boolean)) {
    throw new Error(
      `Nenhum produto encontrado. Verifique a URL e os seletores. Testados: ${productSelectors.join(
        ", "
      )}`
    );
  }
  ...
  const results: ProductResult[] = rawItems
    .map((item) => {
...
app.post("/scrape", async (req, res) => {
  const payload = req.body as ScrapeRequest;
...
app.get("/scrape", async (req, res) => {
  let selectors: ScrapeSelectors | null = null;
  const selectorsRaw = req.query.selectors;
  if (typeof selectorsRaw === "string") {
    try {
      selectors = JSON.parse(selectorsRaw) as ScrapeSelectors;
    } catch {
      res.status(400).json({
    ...
    const discount =
      typeof req.query.discount === "string" ? req.query.discount : undefined;
