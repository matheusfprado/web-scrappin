...
async function run(queryOrUrl) {
    ...
    while (results.length < 50) {
        ...
        const data = await page.evaluate(() => {
            return Array.from(document.querySelectorAll("[data-component-type='s-search-result'], .s-main-slot .s-result-item[data-asin], .s-main-slot .s-card-container")).map((item) => {
                const anchor = item.querySelector("h2 a.a-link-normal") ??
                ...
                const discount = item.querySelector(".a-text-price .a-offscreen")?.textContent?.trim() ??
        ...
        for (const item of data) {
            ...
            if (!Number.isFinite(numericPrice) || numericPrice === Number.POSITIVE_INFINITY)
                continue;
