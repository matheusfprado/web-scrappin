...
function getSheinAuthPath() {
    ...
    return node_path_1.default.join(authDir, 'shein.json');
}
function getSheinUserDataDir() {
    const fromEnv = process.env.SHEIN_USER_DATA_DIR?.trim();
    if (fromEnv)
        return fromEnv;
    const authDir = node_path_1.default.join(process.cwd(), '.auth', 'shein-profile');
    if (!node_fs_1.default.existsSync(authDir)) {
        node_fs_1.default.mkdirSync(authDir, { recursive: true });
    }
    return authDir;
...
async function handleSheinRisk(page, context, headless, authPath, usePersistent) {
    const url = page.url();
    const isRisk = url.includes('/risk/') ||
        url.includes('risk=') ||
        url.includes('/risk/action/limit');
    if (!isRisk)
        return true;
    if (headless) {
        console.log('Bloqueio da Shein detectado. Rode com HEADLESS=false para resolver.');
        return false;
    }
    console.log('Bloqueio da Shein detectado. Resolva a verificacao no navegador.');
...
async function run(queryOrUrl) {
    ...
    if (usePersistent) {
        ...
        const browser = await playwright_1.chromium.launch({ headless, args: launchArgs });
        context = await browser.newContext({
            userAgent,
            viewport: { width: 1280, height: 800 },
            locale: 'pt-BR',
            storageState: node_fs_1.default.existsSync(authPath) ? authPath : undefined,
        });
        closeContext = async () => {
            await browser.close();
        };
    ...
    try {
        ...
        const riskOk = await handleSheinRisk(page, context, headless, authPath, usePersistent);
        if (!riskOk) {
            return;
        }
        const acceptSelectors = [
        ...
        if (debug) {
            const debugInfo = await page.evaluate(() => {
                ...
                const sampleLinks = Array.from(document.querySelectorAll("a[href*='-p-'], a[href*='/p/']"))
        ...
        while (results.length < 50) {
            const waitAttempts = await Promise.all(resultSelectors.map(async (selector) => {
                try {
            ...
            const data = await page.evaluate(() => {
                ...
                return sources.map((item) => {
                    ...
                    const titleFromImg = anchor?.querySelector('img[alt]')?.alt?.trim() ??
                        anchor
                    ...
                    const title = item
                        ...
                        '';
                    const priceText = item.querySelector("[data-testid*='price']")?.textContent?.trim() ??
            ...
            for (const item of data) {
                if (!item.link || !item.title || seenLinks.has(item.link))
                    continue;
                if (!item.price?.trim())
                    continue;
            ...
            if (results.length >= 50)
                break;
            const nextButton = await page.$("a[aria-label='Next'], button[aria-label='Next'], .pagination__next, .sui-pagination__next");
            if (nextButton) {
                await Promise.all([
                    page.waitForLoadState('domcontentloaded'),
                    nextButton.click(),
